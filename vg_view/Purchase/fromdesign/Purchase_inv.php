 ...
 <div id="result">
				...
				<div class="span6">
                  ...
                  <table class="table table-bordered table-invoice" >
                      <tr>
                          <td width="91" class="width30">Purchae  ID:</td>
                          <?php
                          $invid=$this->sql("SELECT IFNULL(MAX(`osm_id`),'0') AS 'invid' FROM `product_accounts` where order_sales_manage_status='PurchaseManage'");
						  $inID=$invid[0]['invid']+1;
						  $str = ($inID < 10000000) ? 0 . $inID : $inID;
						  $osm_CustomCode="Purchase-".date("mYd");
						  ?>
                      ...
                      <tr>
                        <td>Supplier Name</td>
                        <td><select class="form-control required supplier " id="supplierID" name="SupplierNmae" aria-required="true">
  ...
  <table width="100%" class="table table-bordered table-invoice-full">
    ...
    <table width="100%" class="table invoice-table">
          ...
          <div align="right">
    <input type="submit" name="purchasebtn" class="btn btn-primary btn-large" id="purchasebtn" value="Purchase " />          </div>
 </div><!--Result-->
...
<!--```````````````````````````````````````````````````````````````````-->		
	...
	$('body').on('click', '#purchasebtn', function() {
		...
		if($('#Message').val() == '' || $('#osm_content_code').val() == '' || $('#supplierID').val() == '0'){
			jAlert('Please Type Your Message for your Custome Product Manage!', 'Alert');
				...
				var bagSize = [];
					...
					$.each($('.SingleAmount'), function() {
					 ...
					 $.post("<?php echo BASE_URL?>?Purchase_PurchasePD",{
						 Message: $('#Message').val(),
						...
						'payment_amount':$('#paymenttotal').val(),
						 ...
						 function(data){
							 ...
							 switch(data){
								...
								jConfirm('Do you want view repot ?', 'Purchase Report View', function(r) {
    							   
									if(r){
								
			$('#result').load('<?php echo BASE_URL?>?SingelReport_PurchasePDinv&getID='+$('#osmIDFULLL').val()+' #resultt');
										//
										}else{
										location.reload();
											}
