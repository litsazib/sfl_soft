...
<div class="contentinner">
   ...
   $supplier_accounts=$this->sql("SELECT * FROM  `supplier_accounts` WHERE `supplier_id`=".$this->DBH->quote($_GET['getID'])." ORDER BY `create_date` DESC");
   ...
   ?>
           ...
           <?php $supplier= $this->sql("SELECT CONCAT('[ ',`supplier_code`,' ] ',`name`) as supplierName,default_balance FROM  `vg_supplier` WHERE `id_supplier`=".$this->DBH->quote($_GET['getID'])."");
				  echo "<h3>".$supplier[0]['supplierName']."</h3>";
          ...
          <tbody>
			...
			?>        
              <tr class="data-wrapper" <?php echo $corl;?>>
                  ...
                  <td align="center" class="right"><?php echo $supplierResult['net_blance']+$supplier[0]['default_balance'];?></td>
                  <td align="center" class="right"><?php echo $supplierResult['create_date']?></td>
                </tr>
                        
               <?php endforeach;?>
               <?php 
			   if($supplier[0]['default_balance']!=0){
			   ?> 
                <tr class="data-wrapper">
                  <td align="center" class="right">Default Blance</td>
                  <td align="center" class="right">Defalult Amount</td>
                  <td align="center" class="right"><?php echo $supplier[0]['default_balance'];?></td>
