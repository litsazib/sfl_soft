...
SELECT IFNULL(SUM(`total_amount`-`payment_amount`),'0') AS 'netAmount' FROM `customer_accounts` WHERE `customer_id`=".$this->DBH->quote($orderInfo[0]['customer_supplier_id'])."  AND `ref_osm_full_id`!='".$orderInfo[0]['osm_full_id']."' AND `create_date` < '".$orderInfo[0]['create_date']."'");
